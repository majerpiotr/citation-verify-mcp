  ...
  server.registerTool(
    "verify_citations",
    {
      description:
        ...
        "Page and node keywords case-insensitive: a document " +
        ...
        "A bare match after a same-line `://` with no whitespace in between is " +
        "invisible - not resolved, unresolved, or unchecked; scheme-relative " +
        "(`//host/doc.pdf`) and bare-host (`host/doc.pdf`) forms are NOT covered and " +
        "are still read as document names, risking a false `unresolved`. " +
        "A bracket tag - `[node:<id>]` or `[<word>:<id>]` - is `unchecked` UNLESS its " +
        "value names a real `<name>.pdf`; then that document (and any page/node cited " +
        "alongside it) is checked as in prose - otherwise it stays a standalone " +
        "`unchecked` id, never bound to any document (id space unconfirmed): cite the " +
        "real `<name>.pdf` for a verdict. " +
        "A URL-valued tag (`[Source: https://...]`) is not a citation at all. " +
        ...
        "underscores/hyphens only); quoting does not rescue any other character, which " +
        "may instead drop the match entirely (`total: 0`) or leave a shorter, " +
        "not-necessarily-last fragment read as a DIFFERENT document - check `title` " +
        "on a `resolved` verdict to catch this. A quoted span of up to 4 words ending " +
        ...
        "identifies nothing. " +
        "`total: 0` means nothing recognized was found, not a clean bill of " +
        "health; if cited another way, rewrite it in a recognized form and call " +
        "again. Carrying both a " +
